...
void Mqtt::onReceive(Header hdr, Cbor& cbor)
{
	...
	else if (hdr.matches(self(), _connector, REPLY(CONNECT), 0))
	{ // CONNACK received
	...
	else if (hdr.matches(self(), left(), REPLY(CONNECT), 0))
	{ // TCP conected
		_connector.tell(hdr, cbor);
	}
	else if (hdr.matches(self(), left(), REPLY(DISCONNECT), 0))
	{ // TCP disconnected
		_connector.tell(hdr, cbor);
		_pinger.tell(hdr, cbor);
		_publisher.tell(hdr, cbor);
	...
	{ // pinger asks to disconnect after ping failures
		left().tell(self(), DISCONNECT, 0);
	}
	else if (hdr.is(TXD, ANY))
	{	// child wants to transmit, send to framer
		hdr._src = self().idx();
		left().tell(hdr, cbor);
	}
}
...
//			Msg::publish(this, SIG_ERC, 0);
		...
		{	// wrong pubcomp, don't resend PUBREL
			goto WAIT_REPLY1;
		}
	}
}
...
READY:{
	PT_YIELD();
	if ( hdr.is(SUBSCRIBE)) goto SUBSCRIBING;
	if ( hdr.is(RXD,MQTT_MSG_PUBLISH)) {
		_topic.clear();
		_message.clear();
		cbor.offset(0);
		cbor.scanf("iiSB",  &_messageId, &_qos, &_topic, &_message);
		if ( _qos==MQTT_QOS0_FLAG ) {
			out.clear();
			out << _topic << _message;
			right().tell(Header(right(),self(),REPLY(SUBSCRIBE),0),out);
			goto READY;
